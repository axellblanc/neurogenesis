...
import numpy as np
from GDM_master.episodic_gwr import EpisodicGWR
...
def demo(ds):
    ...
    print("%s from %s loaded." % (ds.name, ds.file))
    ...
    e_labels = [3, 3]
    s_labels = [3]
    ds_vectors = ds.vectors
    ds_labels = np.zeros((len(e_labels), len(ds.labels)))
    ds_labels[0] = ds.labels
    ds_labels[1] = ds.labels
    ...
    g_episodic.init_network(ds, e_labels, num_context)
    
    g_semantic = EpisodicGWR()
    g_semantic.init_network(ds, s_labels, num_context)
    ...
    print('\n', g_episodic.num_nodes - 2, ' nodes were added to the episodic memory during the training \n')
    print(g_semantic.num_nodes - 2, ' nodes were added to the semantic memory during the training \n')
