...
class Static:
    ...
    def init_network(self, ds, random, verbose = False, **kwargs) -> None:
        ...
        self.weights = [empty_neuron for j in range(self.num_nodes)]
        
        # Global context
        self.g_context = np.zeros((self.depth, self.dimension))        
        
        # Create habituation counters
        self.habn = [1 for j in range(self.num_nodes)]
        ...
        self.alabels = [empty_label_hist for j in range(self.num_nodes)]
    ...
    def train(self, ds, epochs, a_threshold, beta, l_rates) -> None:
        ...
        self.tau_b = 0.3
        self.tau_n = 0.1
        self.max_nodes = self.num_nodes#self.samples # OK for batch, bad for incremental
        ...
        #print('(-- Added ',self.num_nodes - 2, 'neuron(s) (total added - total deleted)')
    ...
    def test_gammagwr(self, test_ds, **kwargs):
        ...
        self.bmus_index = -np.ones(test_ds.vectors.shape[0])#-np.ones(self.samples)
        self.bmus_label = -np.ones(test_ds.labels.shape[0])#-np.ones(self.samples)
        self.bmus_activation = -np.zeros(test_ds.vectors.shape[0])#np.zeros(self.samples)
